create
database quanLyCuaHangGiayFShoe
go
use quanLyCuaHangGiayFShoe
go
...
alter table traHang
    ...
    go
alter table traHang
    add constraint fk_traHang_sanPhamChiTiet foreign key (sPCT_id) references sanPhamChiTiet (sPCT_id) go
...
('NV001', N'Bùi Văn Hưng', 1, '2004-09-25', N'123 Đường ZXC, Huyện Thạch Thất, Hà Nội', '0987654321', N'Hoạt động', N'duongdananh1.jpg', 'matkhau1', N'Quản lý'),
('NV002', N'Hoàng Đức Anh', 1, '2000-02-22', N'Hà Nội', '0987654322', N'Hoạt động', N'duongdananh2.jpg', 'matkhau2', N'Quản lý'),
('NV003', N'Nguyễn Thị Thùy Dương', 1, '2003-05-13', N'666 Chùa Láng, Hà Nội', '0123456780', N'Nghỉ việc', N'duongdananh3.jpg', 'matkhau3', N'Quản lý'),
('NV004', N'Nguyễn Văn Nam', 1, '2002-12-22', N'101 Đường Xuân Phương, Hà Nội', '0989989898', N'Hoạt động', N'duongdananh4.jpg', 'matkhau4', N'Quản lý'),
('NV005', N'Đỗ Văn E', 1, '1980-04-25', N'321 Đường KLM, Hà Nội', '0888888888', N'Hoạt động', N'duongdananh5.jpg', 'matkhau5', N'Nhân viên'),
('NV006', N'Ngô Thị F', 0, '1997-06-15', N'555 Đường NOP, Hà Nội', '0777777777', N'Hoạt động', N'duongdananh6.jpg', 'matkhau6', N'Nhân viên'),
('NV007', N'Cung Văn G', 1, '1988-10-30', N'888 Đường QRS, Hà Nội', '0666666666', N'Hoạt động', N'duongdananh7.jpg', 'matkhau7', N'Nhân viên'),
('NV008', N'Phạm Thị H', 0, '1993-02-12', N'666 Đường TUV, Hà Nội', '0555555555', N'Hoạt động', N'duongdananh8.jpg', 'matkhau8', N'Nhân viên');
go
...
CREATE
OR
ALTER PROCEDURE ThemSanPhamVaoHoaDonChiTiet
...
@TrangThaiHDCT nvarchar(100);
	set
@GiaBan = (select donGia  from sanPhamChiTiet where sPCT_id like @MaSanPhamChiTiet)
   ...
   --Tính thành tiền = số lượng x giá bán
    ...
    -- Thêm sản phẩm vào hóa đơn chi tiết
		 IF
EXISTS (SELECT 1 FROM HoaDonChiTiet WHERE hoaDon_id = @MaHoaDon AND sPCT_id = @MaSanPhamChiTiet and trangThaiHDCT like N'Chờ thanh toán')
BEGIN
...
@tongTien decimal(18,2);
	set
@tongTien = (select SUM(giaBan*soLuong) from hoaDonChiTiet where trangThaiHDCT like N'Chờ thanh toán' and hoaDon_id = @MaHoaDon)
Update hoaDon
...
@SOLUONG INT;
	SET
@SOLUONG = (SELECT SOLUONG FROM hoaDonChiTiet WHERE hoaDon_id = @MAHOADON and @MAHDCT = hDCT_id and sPCT_id = @MASPCT and trangThaiHDCT = N'Chờ thanh toán')
UPDATE sanPhamChiTiet
set soLuong   = soLuong + @SOLUONG,
...
@THANHTIEN DECIMAL(18,2);
	...
	SET
@DONGIA = (SELECT donGia FROM sanPhamChiTiet WHERE sPCT_id = @MASPCT)
...
UPDATE sanPhamChiTiet
SET soLuong   = (@SOLUONGSP + @SOLUONGSPTRONGGIO) - @SOLUONGSPTHAYDOI,
    trangThai = CASE
                    WHEN ((@SOLUONGSP + @SOLUONGSPTRONGGIO) - @SOLUONGSPTHAYDOI) > 0 THEN N'Còn hàng'
                    ELSE N'Hết hàng'
        END
WHERE sPCT_id = @MASPCT SET @THANHTIEN = @DONGIA * @SOLUONGSPTHAYDOI

--- update lại số lượng sản phẩm trong hoá đơn
UPDATE hoaDonChiTiet
set soLuong   = @SOLUONGSPTHAYDOI,
    thanhTien = @THANHTIEN
WHERE hoaDon_id = @MAHOADON
  AND hDCT_id = @MAHDCT
  AND sPCT_id = @MASPCT
...
UPDATE hoaDonChiTiet
SET trangThaiHDCT = N'Hoàn thành'
where trangThaiHDCT = N'Chờ thanh toán'
  and hoaDon_id = @MAHOADON
END
GO
...
BEGIN
	Declare
@GIATRA decimal(18,2), @TONGTIENHOANTRA DECIMAL(18,2);
	SET
@GIATRA = (SELECT giaBan FROM hoaDonChiTiet WHERE hDCT_id = @MAHOADONCHITIET)
...
UPDATE hoaDonChiTiet
SET thanhTien = soLuong * giaBan
where hDCT_id = @MAHOADONCHITIET IF (SELECT soLuong  FROM hoaDonChiTiet WHERE hDCT_id = @MAHOADONCHITIET )= 0
BEGIN
UPDATE hoaDonChiTiet
SET trangThaiHDCT = N'Xoá'
where hDCT_id = @MAHOADONCHITIET
